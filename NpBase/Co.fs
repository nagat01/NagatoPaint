...
/// int -> Color
let ofInt i =
  let f n = i / pown 256 n |> byte
  Color.FromArgb(f 3, f 2, f 1, f 0)
...
// RGB値のどれかを得る
let toA (co:Color) = co.A
let toR (co:Color) = co.R
let toG (co:Color) = co.G
let toB (co:Color) = co.B

// RGB値のどれかを変更する
let mapA f (co:Color) = Color.FromArgb(f co.A, co.R, co.G, co.B)
let mapR f (co:Color) = Color.FromArgb(co.A, f co.R, co.G, co.B)
let mapG f (co:Color) = Color.FromArgb(co.A, co.R, f co.G, co.B)
let mapB f (co:Color) = Color.FromArgb(co.A, co.R, co.G, f co.B)

/// float型のRGB値からColorを作る
let ofFloats a r g b = 
  Color.FromScRgb(float32 a, float32 r, float32 g, float32 b)
...
let fromHsl h s l : Color =
  ...
  if v = 0.0 then ofFloats 1. l l l else
    let m = l * 2.0 - v
    let sv = (v - m) / v
    let h = h * 6.0
    let sextant = int h
    let fract = Math.fraction h
    let vsf = v * sv * fract
    let mid1 = m + vsf
    let mid2 = v - vsf
    match sextant with
